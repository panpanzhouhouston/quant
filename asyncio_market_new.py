...
class Broker(object):
    ...
    def NEW_ORDER_ON_REQUEST(self, requenst):
...
def task_generator(account_detail, all_instrument_dict, ticker):
    deliver_price = all_instrument_dict[ticker]['deliver_price']
    bid_level = [0,0,0]
    ask_level = [0,0,0]
    task = []
    for order in account_detail['orders'][(ticker, 0, 0)]:
        if deliver_price - order[1] < 0.08 or deliver_price - order[1] > 0.11:
            task.append(('cancel', order[0]))
        elif round(deliver_price - order[1],2) == 0.08:
            bid_level[0] += order[2]
        elif round(deliver_price - order[1],2) == 0.09:
            bid_level[1] += order[2]
    ...
    for key, val in account_detail['long_pos'].items():
        if val[1] > 800 and key == 'A002.PSE':
            task.append(('order', key, 'ASK', val[1]-800, 100, 'LONG', True))
        elif val[1] > 400:
            task.append(('order', key, 'ASK', val[1]-400, 100, 'LONG', True))

    for key, val in account_detail['short_pos'].items():
        if val[1] > 800 and key == 'A002.PSE':
            task.append(('order', key, 'BID', val[1] - 800, 100, 'SHORT', True))
        elif val[1] > 400:
            task.append(('order', key, 'BID', val[1] - 400, 100, 'SHORT', True))
    

    for i in range(3):
        if bid_level[i] < 5:
            #print('adding bid tick at ', DEL_PRC - 0.08 - 0.01*i, ' with vol', 5-bid_level[i])
            task.append(('order', ticker,'BID', 5-bid_level[i], deliver_price - 0.08 - 0.01*i, 'LONG', False))
            
        if ask_level[i] < 5:
            #print('adding ask tick at ', DEL_PRC + 0.08 + 0.01*i, ' with vol', 5-ask_level[i])
            task.append(('order', ticker,'ASK', 5-ask_level[i], deliver_price + 0.08 + 0.01*i, 'SHORT', False))
        ...
        for item in data:
            ...
            if i % 10 == 0:
                ...
                for ticker in ['A001.PSE', 'A002.PSE', 'B001.PSE', 'B002.PSE']:
                    tasks.extend(task_generator(account_detail, all_instrument_dict, ticker))
