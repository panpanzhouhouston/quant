...
def task_generator(account_detail, all_instrument_dict, ticker, long_ave, short_ave):
    ...
    if ticker =='B001.PSE':
        for order in account_detail['orders'][(ticker, 0, 0)]:
            if price - order[1] < 0.089 or price - order[1] > 0.101:
    ...
    elif ticker == 'B002.PSE':
        for order in account_detail['orders'][(ticker, 0, 0)]:
            if price - order[1] < 0.079 or price - order[1] > 0.091:
                task.append(('cancel', order[0]))
    ...
    if ticker == 'B001.PSE':
        for i in range(2):
            ...
            if bid_level[i] < 8:
                ...
                task.append(('order', ticker, 'BID', 8 - bid_level[i], price - 0.09 - 0.01 * i, 'LONG', False))

            if ask_level[i] < 8:
                # print('adding ask tick at ', DEL_PRC + 0.08 + 0.01*i, ' with vol', 5-ask_level[i])
                task.append(('order', ticker, 'ASK', 8 - ask_level[i], price + 0.09 + 0.01 * i, 'SHORT', False))
        ...
        for i in range(2):
            ...
            if bid_level[i] < 8:
                # print('adding bid tick at ', DEL_PRC - 0.08 - 0.01*i, ' with vol', 5-bid_level[i])
                task.append(('order', ticker, 'BID', 8 - bid_level[i], price - 0.08 - 0.01 * i, 'LONG', False))

            if ask_level[i] < 8:
                # print('adding ask tick at ', DEL_PRC + 0.08 + 0.01*i, ' with vol', 5-ask_level[i])
                task.append(('order', ticker, 'ASK', 8 - ask_level[i], price + 0.08 + 0.01 * i, 'SHORT', False))

    elif ticker == 'A001.PSE':
        if bid_level[1] < 15:
            task.append(('order', ticker, 'BID', 15 - bid_level[1], price - 0.1, 'LONG', False))

        if ask_level[1] < 15:
            task.append(('order', ticker, 'ASK', 15 - ask_level[1], price + 0.1, 'SHORT', False))

    else:
        if bid_level[1] < 15:
            task.append(('order', ticker, 'BID', 15 - bid_level[1], price - 0.09, 'LONG', False))

        if ask_level[1] < 15:
            task.append(('order', ticker, 'ASK', 15 - ask_level[1], price + 0.11, 'SHORT', False))
